...
await describe('Duels Division Calculation', async () => {
  await describe('Short Mode (standard modes)', async () => {
    await it('should return Unranked for less than 50 wins', () => {
      assert.equal(calculateDuelsDivision(0, 'short'), 'Unranked')
      assert.equal(calculateDuelsDivision(49, 'short'), 'Unranked')
    })

    await it('should calculate Rookie divisions correctly', () => {
    ...
    await it('should calculate Iron divisions correctly', () => {
  ...
  await describe('Long Mode (Bridge, Boxing, MegaWalls, NoDebuff, Parkour)', async () => {
    await it('should return Unranked for less than 25 wins', () => {
    ...
    await it('should calculate Divine divisions correctly', () => {
      ...
      assert.equal(calculateDuelsDivision(26_688, 'long'), 'Divine I')
  ...
  await describe('Overall Stats', async () => {
    await it('should return Unranked for less than 100 wins', () => {
      assert.equal(calculateDuelsDivision(99, 'overall'), 'Unranked')
    })

    await it('should calculate Rookie divisions correctly (2x multiplier)', () => {
  ...
  await describe('Edge Cases', async () => {
    ...
    await it('should handle boundary values correctly - Long mode', () => {
      assert.equal(calculateDuelsDivision(24, 'long'), 'Unranked')
      assert.equal(calculateDuelsDivision(25, 'long'), 'Rookie I')
      assert.equal(calculateDuelsDivision(50, 'long'), 'Iron I')
    ...
    await it('should handle boundary values correctly - Overall', () => {
      assert.equal(calculateDuelsDivision(99, 'overall'), 'Unranked')
      assert.equal(calculateDuelsDivision(100, 'overall'), 'Rookie I')
