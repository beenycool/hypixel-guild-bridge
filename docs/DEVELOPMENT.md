...
`config`: Generated directory made by the application during its runtime. Local runtime files and backups are saved there. Dynamic configuration and runtime state are stored in PostgreSQL, so local/dev runs must still configure `database.url` or `DATABASE_URL` unless they explicitly opt into `memory://local`.  
...
This defines the Public API of the application that all Instances use to communicate with each other.  
...
Check files such as `src/common/application-event.ts` and `src/common/commands.ts` for more up-to-date info.
