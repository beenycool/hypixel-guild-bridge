...
export default class Tnttag extends HypixelPlayerCommand {
  ...
  async onPlayer(context: ChatCommandContext, givenUsername: string, player: Player): Promise<string> {
    ...
    if (tntGames === undefined) return `${givenUsername} has never played TNT Tag.`

    const tnttag = tntGames.tnttag
    if (!tnttag || tnttag.wins === undefined) return `${givenUsername} has never played TNT Tag.`
    ...
    return `${givenUsername}'s TNT Tag: Wins: ${shortenNumber(wins)} | Kills: ${shortenNumber(kills)} | Deaths: ${shortenNumber(deaths)} | K/D: ${kdr}`
  }
}
